package com.emirates.skybook.cordova.plugin.printer;
...
import com.emirates.skybook.cordova.plugin.printer.utils.DemoSleeper;
...
/**
 * Will fetch the version from ApplicationMainFest.xml file
 */
public class PrinterInterface extends CordovaPlugin implements DiscoveryHandler {
	...
	private static final String SEATNUMBER = "seatNumber";
	...
	CallbackContext discoveryCallbackContext;
	CallbackContext printerCallbackContext;
					...
					cordova.getThreadPool().execute(new Runnable() {
			            public void run() {
			            	try {
			            		...
			            		System.out.println("order data "+obj.getJSONArray("printdata"));
			            		JSONArray item = obj.getJSONArray("printdata");
			            		
			            		System.out.println("order data "+item.toString());
								print(item);
	...
	public void discoverPrinters(String deviceIpAddress) throws JSONException {
		...
		System.out.println(subnetRange);

		try {
			NetworkDiscoverer.subnetSearch(this, subnetRange);
		} catch (DiscoveryException e) {
			System.out.println("Error in discovery" + e.getMessage());
			JSONObject result = new JSONObject();
	...
	public void discoveryError(String message) {
		...
		System.out.println("Error in printer dicsovery: " + message);
		JSONObject result = new JSONObject();
		try {
	...
	public void discoveryFinished() {
		// TODO Auto-generated method stub
		System.out.println("Printers found: " + printersFound.size());
		JSONObject result = new JSONObject();
		try {
	...
	public void foundPrinter(final DiscoveredPrinter printerDetails) {
		// TODO Auto-generated method stub
		System.out.println("Printers found adding it");
			...
			if (printerConnection.isConnected()) {
				...
				if (printer != null) {
					...
					if (pl == PrinterLanguage.CPCL) {
						// helper.showErrorDialogOnGuiThread("This demo will not
						// work for CPCL printers!");
						...
						if(printer.getCurrentStatus().isPaperOut){
							...
							for(int i =0; i < item.length(); i++){
								...
								System.out.println("seatNo "+seatNo+", "+"PaxName "+paxName+", "+"orderData "+orderData);
	...
	private String createZplReceipt(Map<String, String> printData) {
		...
		System.out.println("print names "+firstSection+"-"+secondSection+"="+lastIndex+"-"+paxName.length());
		int headerHeight = 160;
		if (secondSection != "" ) {
			// TODO write the logix here to beak the pax name into chunk of 23
			// characters
			// and "^FO 20,110" + "\r\n" + "^A0,N,35,35" + "\r\n" + "^FD%s^FS" +
			// "\r\n" + format
		...
		for (String productName : itemsToPrint) {
			...
			body += String.format(lineItem, totalHeight+5, totalHeight, productName);
		}

		long totalBodyHeight = (itemsToPrint.length + 1) * heightOfOneLine;
		...
		System.out.println("Data to be printed: " + wholeZplLabel);
